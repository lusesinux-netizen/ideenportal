...
import { Calendar, Users, ArrowRight, Sparkles, Loader2 } from 'lucide-react';
...
export default function SuggestionCard({ suggestion, index = 0 }: { suggestion: DbSuggestion; index?: number }) {
  ...
  const handleSummary = async (e: React.MouseEvent) => {
    ...
    const result = await getSummary(suggestion);
        ...
        <div className="group rounded-xl border bg-card p-5 shadow-card transition-all hover:shadow-elevated hover:-translate-y-0.5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              {summary ? (
                <p className="mt-1 text-sm text-primary/80 italic flex items-start gap-1.5">
